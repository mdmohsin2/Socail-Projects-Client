...
const Media = () => {
    ...
    return (
        <div data-theme="night" className=''>
            {
                posts.map(post=><div key={post._id} className="flex flex-col mx-auto max-w-lg p-6 space-y-6 overflow-hidden rounded-lg shadow-md dark:bg-gray-900 dark:text-gray-100">
                <div className="flex space-x-4">
                    <img alt="profile" src={user?.photoURL} className="object-cover w-12 h-12 rounded-full shadow dark:bg-gray-500" />
                    <div className="flex flex-col space-y-1">
                        <a rel="noopener noreferrer" href="/" className="text-sm font-semibold text-white">{user?.displayName}</a>
                        {/* <span className="text-xs dark:text-gray-400">4 hours ago</span> */}
                    </div>
                </div>
                <div>
                    <img src={post.image} alt="" className="object-cover w-full mb-4 h-60 sm:h-96 dark:bg-gray-500" />
                    <h2 className="mb-1 text-xl font-semibold">{post.message}</h2>
